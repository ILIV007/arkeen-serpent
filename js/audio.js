...
const Audio = (function(){
  ...
  function playEat(){ init(); resume(); pluck(880, 0.07, 0.25); }
  function playGolden(){ init(); resume(); pluck(1320, 0.12, 0.3); pluck(1760, 0.1, 0.2, 0.05); }
  function playPoison(){ init(); resume(); noise(0.15, 0.2, 200); tone(150, 0.2, 'sawtooth', 0.1); }
  function playCombo(n){
    init(); resume();
    for(let i=0;i<Math.min(n,4);i++) pluck(660 + i*220, 0.06, 0.2, i*0.04);
  }
  function playLevelUp(){ init(); resume(); tone(523, 0.1, 'sine', 0.3); tone(659, 0.1, 'sine', 0.3, 0.08); tone(784, 0.15, 'sine', 0.3, 0.16); }
  function playGameOver(){
    init(); resume();
    tone(400, 0.2, 'sawtooth', 0.2);
    tone(300, 0.3, 'sawtooth', 0.2, 0.15);
    tone(200, 0.4, 'sawtooth', 0.2, 0.3);
  }
  ...
  function startAmbient(){
    ...
    ambientGain = ctx.createGain(); ambientGain.gain.value = 0.08;
    const f = ctx.createBiquadFilter(); f.type='lowpass'; f.frequency.value = 400;
    ambientNode.connect(f); f.connect(ambientGain); ambientGain.connect(masterGain);
    ambientNode.start();
  }
  function stopAmbient(){ if(ambientNode){ ambientNode.stop(); ambientNode=null; ambientGain=null; } }
