...
function drawMenuBackground(ctx, theme) {
  ...
  ctx.beginPath();
  for (let i = 0; i < 60; i++) {
    const x = canvas.width / 2 + Math.sin(t + i * 0.15) * 120 + Math.cos(t * 0.4 + i * 0.1) * 60;
    const y = canvas.height / 2 + Math.cos(t + i * 0.12) * 100 + Math.sin(t * 0.3 + i * 0.08) * 50;
...
function drawGame(ctx, theme) {
  ...
  state.snake.forEach((seg, i) => {
    ...
    const isHead = i === 0;
    const size = isHead ? cs * 0.9 : cs * 0.78;
    const pad = (cs - size) / 2;
    ...
    const alpha = isHead ? 1 : 0.65 + (state.snake.length - i) / state.snake.length * 0.35;
    ...
    if (isHead) {
      ...
      switch (state.direction) {
        case 'RIGHT':
          ex1 = sx + cs * 0.72; ey1 = sy + cs * 0.28;
          ex2 = sx + cs * 0.72; ey2 = sy + cs * 0.72;
          break;
        case 'LEFT':
          ex1 = sx + cs * 0.28; ey1 = sy + cs * 0.28;
          ex2 = sx + cs * 0.28; ey2 = sy + cs * 0.72;
          break;
        case 'UP':
          ex1 = sx + cs * 0.28; ey1 = sy + cs * 0.28;
          ex2 = sx + cs * 0.72; ey2 = sy + cs * 0.28;
          break;
        case 'DOWN':
          ex1 = sx + cs * 0.28; ey1 = sy + cs * 0.72;
          ex2 = sx + cs * 0.72; ey2 = sy + cs * 0.72;
          break;
      }
      ctx.beginPath();
...
function drawAmbient(ctx, theme) {
  if (theme.ambient === 'stars') {
    ...
    drawStars(ctx); // default for void too
  }
}
...
function drawStars(ctx) {
  ...
  for (const s of stars) {
    const flicker = 0.5 + Math.sin(t * s.twinkleSpeed + s.x) * 0.5;
...
function drawFireGlow(ctx) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    grad.addColorStop(0, 'rgba(255, 50, 0, 0.07)');
    grad.addColorStop(1, 'transparent');
    ctx.fillStyle = grad;
    ctx.beginPath();
...
function drawNebula(ctx) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const colors = ['rgba(176, 132, 255, 0.05)', 'rgba(128, 80, 204, 0.04)', 'rgba(0, 255, 170, 0.03)'];
