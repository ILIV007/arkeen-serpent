/**
 * STORAGE — Arkeen Serpent
 * Leaderboard, Statistics, Settings, Achievements
 */
...
export const Leaderboard = {
  ...
  add(name, score) {
    const lb = this.get();
    lb.push({ name: String(name || 'KNIGHT').substring(0, 8).toUpperCase(), score: Number(score) || 0, date: new Date().toLocaleDateString() });
  ...
  isHigh(score) {
    ...
    return score > (lb[lb.length - 1]?.score || 0);
  },
  rank(score) {
    const lb = this.get();
    let rank = 1;
